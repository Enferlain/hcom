...
workspace without repeatedly asking permission and without receiving ambient
write access to the rest of the host.
...
hcom claude --headless --isolate workspace ...
...
| `workspace` | Workspace writable, Git metadata read-only, private temp/cache, scoped hcom state |
| `workspace-git` | As above, but Git metadata writable for explicitly authorized worker branches |
...
sandbox
  ...
  workspace-scoped hcom client and hooks
```
...
2. `IsolationPlan` contains only canonical, validated paths and environment
   entries. It is serializable for launch records and diagnostics.
...
5. Cleanup owns the provider process group and per-run runtime directory.
...
Milestone 1 covers the `HeadlessPty` backend used by `hcom run agy` and the
Claude-backed `hcom run glm` workflow. Foreground PTY should use the same spawn
function once the headless path is proven.
...
- a per-workflow hcom directory containing no global config, scripts, archives,
...
It should disable nested user namespaces when supported. Mount targets are
created only beneath hcom-owned runtime roots before untrusted workspace content
is introduced; symlink and canonical-path validation is a hard precondition.
...
Milestone 1 creates a private hcom directory per workflow. The host-side
coordinator and sandboxed participants use it for the workflow database, hooks,
result event, and transcript references. The high-level wrapper returns the
correlated result over stdout, so the caller does not need direct membership in
the private database.

The directory and launch record are keyed by explicit workflow and attempt IDs;
the agent instance is a participant in that workflow rather than the owner of
its security scope. This gives later task, retry, and reviewer orchestration a
stable boundary without requiring a broad workflow-schema migration in the
first release.

This is sufficient for isolated `hcom run` delegation and worker/reviewer
pipelines. General communication between an isolated worker and arbitrary
agents in the global hcom database requires a later broker. That broker
authorizes a small protocol by workflow, worker generation, thread, and message
intent; it must not proxy arbitrary database queries or filesystem access.
...
available. Copying a user's complete provider directory into the sandbox is not
permitted. A provider that cannot start without exposing a durable credential
remains unsupported by the hardened profile until that path is tested.
...
`workspace` must not be called hardened while using `host` networking. Brokered
egress is a release gate for untrusted repositories or issue text. For the
immediate trusted-repository workflow, filesystem/process isolation with
explicit `host` network status may be released as a preview after the user
...
There is no fallback from `workspace` to `off`. Resume reuses the recorded
profile and canonical workspace or fails. Kill and timeout cleanup terminate the
namespace/process group before deleting runtime state. Cleanup warnings after an
authoritative task result follow issue 45's result-preserving semantics.
...
- private temp and declared caches work;
- sandboxed hcom completion reaches the host coordinator;
...
6. Worker/reviewer workflow using one private hcom workflow directory.
...
- Support fake providers and PTY-backed real providers with filesystem/process
  isolation.
- Create per-workflow hcom state and correlated result return.
...
- Remove provider workspace-trust and command-approval loops from the normal
  isolated workflow without changing durable provider trust outside it.

### Phase 3: hardened egress and broker

- Add credential-aware and network-aware brokers.
- Add the narrow global-hcom communication bridge.
...
1. It supports trusted local repositories and reports `network=host` on every
   launch. Untrusted repositories and GitHub-event content remain blocked until
   brokered egress and credentials are implemented.
...
3. Isolated results are exposed first through high-level `hcom run` workflows.
   Raw cross-database agent messaging waits for the narrow broker.

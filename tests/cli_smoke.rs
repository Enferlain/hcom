...
fn list_masks_filter_wait_context() {
    ...
    let db = rusqlite::Connection::open(h.hcom_dir.join("hcom.db")).unwrap();
    db.execute(
        "UPDATE instances SET status = 'listening', status_context = 'filter-wait:abc:def', status_time = ?2 WHERE name = ?1",
